...
use persist_core::{
    create_default_engine, SnapshotEngine, SnapshotMetadata,
    LocalFileStorage, GzipCompressor, NoCompression,
};
    ...
    for size_kb in [1, 10, 100, 1000].iter() {
        ...
        group.bench_with_input(
            ...
            |b, &size_kb| {
                ...
                b.iter(|| {
                    let file_path = temp_dir.path().join(format!("bench_save_{}_{}.json.gz", size_kb, rand::random::<u32>()));
                    black_box(engine.save_snapshot(
                        black_box(&data),
                        black_box(&metadata),
...
fn benchmark_load_operations(c: &mut Criterion) {
    ...
    for size_kb in [1, 10, 100, 1000].iter() {
        let data = generate_test_data(*size_kb);
        let metadata = SnapshotMetadata::new("benchmark_agent", "benchmark_session", 0);
        let file_path = temp_dir.path().join(format!("bench_load_{}.json.gz", size_kb));
    ...
    for (file_path, size_kb, data_len) in snapshot_paths {
        ...
        group.bench_with_input(
            ...
            |b, file_path| {
                b.iter(|| {
                    black_box(engine.load_snapshot(black_box(file_path.to_str().unwrap())).unwrap());
                });
            },
        );
    }
...
fn benchmark_compression_algorithms(c: &mut Criterion) {
    ...
    group.bench_function("gzip_default", |b| {
        b.iter(|| {
            let file_path = temp_dir.path().join(format!("gzip_default_{}.json.gz", rand::random::<u32>()));
            black_box(gzip_engine.save_snapshot(
                black_box(&data),
                black_box(&metadata),
                black_box(file_path.to_str().unwrap())
            ).unwrap());
    ...
    group.bench_function("gzip_fast", |b| {
        b.iter(|| {
            let file_path = temp_dir.path().join(format!("gzip_fast_{}.json.gz", rand::random::<u32>()));
            black_box(gzip_fast_engine.save_snapshot(
                black_box(&data),
                black_box(&metadata),
                black_box(file_path.to_str().unwrap())
            ).unwrap());
    ...
    group.bench_function("gzip_max", |b| {
        b.iter(|| {
            let file_path = temp_dir.path().join(format!("gzip_max_{}.json.gz", rand::random::<u32>()));
            black_box(gzip_max_engine.save_snapshot(
                black_box(&data),
                black_box(&metadata),
                black_box(file_path.to_str().unwrap())
            ).unwrap());
    ...
    group.bench_function("no_compression", |b| {
        b.iter(|| {
            let file_path = temp_dir.path().join(format!("no_comp_{}.json", rand::random::<u32>()));
            black_box(no_comp_engine.save_snapshot(
                black_box(&data),
                black_box(&metadata),
                black_box(file_path.to_str().unwrap())
            ).unwrap());
...
fn benchmark_parallel_operations(c: &mut Criterion) {
    ...
    group.bench_function("sequential_save", |b| {
        b.iter(|| {
            for (i, (data, metadata, _)) in agents.iter().enumerate() {
                let file_path = temp_dir.path().join(format!("seq_save_{}_{}.json.gz", i, rand::random::<u32>()));
                black_box(engine.save_snapshot(
                    black_box(data),
                    black_box(metadata),
                    black_box(file_path.to_str().unwrap())
    ...
    group.bench_function("parallel_save", |b| {
        b.iter(|| {
            agents.par_iter().enumerate().for_each(|(i, (data, metadata, _))| {
                let file_path = temp_dir.path().join(format!("par_save_{}_{}.json.gz", i, rand::random::<u32>()));
                black_box(engine.save_snapshot(
                    black_box(data),
                    black_box(metadata),
                    black_box(file_path.to_str().unwrap())
...
fn benchmark_memory_usage(c: &mut Criterion) {
    ...
    for size_kb in [100, 500, 1000, 5000].iter() {
        ...
        group.bench_with_input(
            ...
            |b, &size_kb| {
                b.iter(|| {
                    let file_path = temp_dir.path().join(format!("memory_test_{}_{}.json.gz", size_kb, rand::random::<u32>()));
                    black_box(engine.save_snapshot(
                        black_box(&data),
                        black_box(&metadata),
                        black_box(file_path.to_str().unwrap())
    ...
    for size_kb in [10, 100, 1000].iter() {
        ...
        group.bench_with_input(
            ...
            |b, &size_kb| {
                ...
                b.iter(|| {
                    let file_path = temp_dir.path().join(format!("roundtrip_{}_{}.json.gz", size_kb, rand::random::<u32>()));
                    ...
                    // Load
                    let (loaded_metadata, loaded_data) = black_box(engine.load_snapshot(
                        black_box(file_path.to_str().unwrap())
                    ).unwrap());
