    ...
    C: CompressionAdapter,
{
    ...
    /// 2. Computes the content hash and updates metadata
    /// 3. Creates a snapshot container with metadata and agent state
    /// 4. Serializes the container to JSON
    /// 5. Compresses the JSON data
    /// 6. Saves the compressed data using the storage adapter
    ///
    /// # Arguments
    ...
    pub fn save_snapshot(
        ...
        // Update metadata with content hash and size information
        let agent_bytes = agent_json.as_bytes();
    ...
    pub fn load_snapshot(&self, path: &str) -> Result<(SnapshotMetadata, String)> {
        ...
        // Convert agent state back to JSON string
