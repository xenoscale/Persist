...
use persist_core::{
    SnapshotEngine, SnapshotMetadata, LocalFileStorage, GzipCompressor,
    create_default_engine, PersistError
};

/// Convert a Rust PersistError to a Python exception
impl From<PersistError> for PyErr {
...
fn restore(
    ...
    let py_agent = if let Some(secrets) = secrets_map {
        loads_func.call((agent_json, secrets))
