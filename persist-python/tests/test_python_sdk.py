...
def sample_agent_data():
    ...
    return {
        ...
        "state": {"active": True, "session_id": "test_session_123"},
    }
    ...
    def test_performance_benchmarks(self, temp_dir):
        ...
        for size_name, target_size in data_sizes:
            ...
            data = {
                ...
                "items": [
                    f"Item {i} with some content to reach target size" for i in range(num_items)
                ],
            }
        ...
        print("-" * 70)
        print(
            f"{'Size':<10} {'Data Size':<12} {'File Size':<12} {'Save Time':<10} {'Load Time':<10} {'Compression':<12}"
        )
        print("-" * 70)
        ...
        for size_name, results in performance_results.items():
            assert (
                results["save_time"] < 5.0
            ), f"Save time too slow for {size_name}: {results['save_time']}s"
            assert (
                results["load_time"] < 3.0
            ), f"Load time too slow for {size_name}: {results['load_time']}s"
