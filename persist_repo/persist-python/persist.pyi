...
def snapshot(
    ...
    snapshot file. Supports local filesystem, Amazon S3, and Google Cloud Storage backends.
    ...
    Args:
        ...
        storage_mode: Storage backend - "local", "s3", or "gcs" (default: "local")
        s3_bucket: S3 bucket name (required for S3 mode)
        s3_region: S3 region (optional, uses AWS environment default)
        gcs_bucket: GCS bucket name (required for GCS mode)
        gcs_credentials_path: Path to GCP service account JSON file (optional, uses ADC if not provided)
    ...
    Example:
        ...
        >>> persist.snapshot(agent, "agent1/session1/snapshot.json.gz",
        ...                 storage_mode="gcs",
        ...                 gcs_bucket="my-snapshots-bucket",
        ...                 agent_id="conversation_agent")
    """
    ...
