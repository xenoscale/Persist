...
fn create_storage_config(
    ...
    gcs_credentials_path: Option<&str>,
    ...
    match mode.as_str() {
        ...
        "gcs" => {
            ...
            let mut config = if let Some(bucket) = gcs_bucket {
                StorageConfig::gcs_with_bucket(bucket.to_string())
            } else {
                StorageConfig::default_gcs()
            };

            if let Some(creds_path) = gcs_credentials_path {
                config.gcs_credentials_path = Some(PathBuf::from(creds_path));
            }

            Ok(config)
        }
        _ => Err(PyIOError::new_err(format!(
            "Invalid storage_mode '{mode}'. Must be 'local', 's3', or 'gcs'"
        ))),
    }
}
...
/// ```
#[pyfunction]
#[pyo3(signature = (agent, path, agent_id="default_agent", session_id="default_session", snapshot_index=0, description=None, storage_mode=None, s3_bucket=None, s3_region=None, gcs_bucket=None, gcs_credentials_path=None))]
...
/// ```
#[pyfunction]
#[pyo3(signature = (path, secrets_map=None, storage_mode=None, s3_bucket=None, s3_region=None, gcs_bucket=None, gcs_credentials_path=None))]
#[allow(clippy::too_many_arguments)]
fn restore(
    py: Python<'_>,
    path: &str,
...
#[pyfunction]
#[pyo3(signature = (path, storage_mode=None, s3_bucket=None, s3_region=None, gcs_bucket=None, gcs_credentials_path=None))]
#[allow(clippy::too_many_arguments)]
fn get_metadata(
    py: Python<'_>,
    path: &str,
